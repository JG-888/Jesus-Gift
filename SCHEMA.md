...
Append exactly one line per **closed** trade:
```json
{"id":"<uuid>","ts_open":"<ISO8601>","ts_close":"<ISO8601>","symbol":"BTC/USD","side":"short","qty":0.0,"entry":0.0,"exit":0.0,"pnl":0.0,"pnl_pct":0.0,"stop":0.0,"target":0.0,"timeframe":"1m","reason_entry":"","reason_exit":""}
```

## `memory/insights.jsonl` — append-only, one JSON object per line
Append a line whenever the agent learns something worth keeping:
```json
